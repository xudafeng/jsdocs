...
proto.init = function(){
  ...
  if(!fileUtil.isExistedDir(that.output)){
    mkdirp(that.output);
  }

  fileUtil.traversal(that.input ,function(error, result){
    if (error){
      throw error;
    }
...
proto.createFile = function(data){
  ...
  var that = this;
  var dist = join(that.output, path.relative(that.input ,data.dir), data.name);
